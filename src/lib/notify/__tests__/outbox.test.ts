...
describe('failure handling', () => {
  ...
  it('requeues a failure and sends it on the next flush', async () => {
    ...
    // Back to mock, as if the provider recovered.
    delete process.env.NOTIFY_SOURCE
...
describe('provider selection', () => {
  ...
  it('falls back to mock when resend is forced without a key', async () => {
    // A missing key must never mean a member's plan changed and the send threw.
    process.env.NOTIFY_SOURCE = 'resend'
    const { getNotificationSource } = await import('@/lib/notify')
    expect(getNotificationSource()).toBe('mock')
  })
})
