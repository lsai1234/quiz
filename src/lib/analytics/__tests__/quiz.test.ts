...
describe('funnel', () => {
  ...
  it('checkout start reuses the shop name but tags source: quiz', () => {
  ...
  it('does not emit a pre-payment success event', () => {
    // `checkout_success` fired before the member had paid; the server-verified
    // `purchase` event on /order/confirmation replaces it.
